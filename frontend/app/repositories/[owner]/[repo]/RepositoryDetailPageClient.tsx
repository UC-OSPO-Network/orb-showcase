...
type RepositoryDetailPageClientProps = {
  apiUrl: string;
};

export default function RepositoryDetailPageClient({
  apiUrl,
}: RepositoryDetailPageClientProps) {
  ...
  const {
    ...
    queryFn: async () => {
      const res = await fetch(`${apiUrl}/repositories/${owner}/${repo}`);
      if (res.status === 404) {
  ...
  const {
    ...
    queryFn: async () => {
      if (
        ...
        displayRepo.full_name.includes("/")
      ) {
  ...
  const is404 =
    ...
    ((typeof error === "object" &&
      ...
      (error as any).message === "Repository not found");
  ...
  return (
    <>
      <Head>
        ...
        <meta
          name="description"
          content={displayRepo.description || "Repository details"}
        />
      </Head>
      <RepositoryPage
        repo={displayRepo}
